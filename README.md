...
On initialize the server uses `rootUri` (or the first workspace folder), reads already-restored dependency sources from the workspace's `deps/` directory, and invokes Mix plus the standard Elixir parallel compiler with a compiler tracer. It never runs `deps.get`. Mix compiles dependencies and the project in the workspace's conventional `_build/<env>` directory, while the hierarchy index remains in the external user cache.

Workspaces with Erlang/Rebar dependencies require `rebar3` in the active Mix toolchain. Install it explicitly with `mix local.rebar --force`, or, when launching through mise, `mise x -- mix local.rebar --force`. CI may instead provide a pinned executable through `MIX_REBAR3`. The server is noninteractive: when Rebar3 is missing it fails with this guidance and does not install build tools or run `deps.get` automatically.

The cache identity covers cache/indexer schema, Elixir/OTP/Mix, OS and architecture, `MIX_ENV`, workspace identity, and deterministic content digests for `mix.exs`, `mix.lock`, `config/`, `lib/`, and restored `deps/` sources. VCS/build junk, `_build`, and directory symlinks are excluded. Indexes use validated versioned JSON; corrupt entries rebuild atomically. An entry-specific bounded cross-process lock performs a second hit check, so a valid hit loads the complete persisted index without compiling dependencies or project files again. The authoritative workspace must be trusted and writable: a cold miss executes compiler and macro code and updates ordinary Mix build artifacts.
